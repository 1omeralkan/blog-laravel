<x-guest-layout>
    ...
    <div class="muk-card">
        <div class="muk-title">Kayıt Ol</div>
        <form method="POST" action="{{ route('register') }}">
            @csrf
            <label class="muk-label" for="name">İsim</label>
            <input id="name" class="muk-input" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mb-2" />
            <label class="muk-label" for="email">Email</label>
            <input id="email" class="muk-input" type="email" name="email" :value="old('email')" required autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mb-2" />
            <label class="muk-label" for="password">Şifre</label>
            <input id="password" class="muk-input" type="password" name="password" required autocomplete="new-password" />
            <x-input-error :messages="$errors->get('password')" class="mb-2" />
            <label class="muk-label" for="password_confirmation">Şifre Tekrar</label>
            <input id="password_confirmation" class="muk-input" type="password" name="password_confirmation" required autocomplete="new-password" />
            <x-input-error :messages="$errors->get('password_confirmation')" class="mb-2" />
            <button type="submit" class="muk-btn">Kayıt Ol</button>
        </form>
